...
class LinkLayer(Prototype):
	...
	def send(self,data):
		...
		if self.waitPacketSent():
			...
			self.PL.writeRegister(RFM_REG_FIFO, 0xff) # broadcast
			self.PL.writeRegister(RFM_REG_FIFO, 0xff) # broadcast
			self.PL.writeRegister(RFM_REG_FIFO, self._TX_id) #
			self.PL.writeRegister(RFM_REG_FIFO, 0) #
	...
	def sendWithHeader(self,data,varTo,varFrom,varFlag,idtx):
		#print('send',data)
		if self.waitPacketSent():
			self.setOpModeIdle()
			...
			#<to> <from> <id> <flags> 
			#self.PL.writeRegister(RFM_REG_FIFO, [0,0,self._TX_id,0])
			self.PL.writeRegister(RFM_REG_FIFO, varTo) # broadcast
			self.PL.writeRegister(RFM_REG_FIFO, varFrom) # broadcast
			self.PL.writeRegister(RFM_REG_FIFO, idtx) #
			self.PL.writeRegister(RFM_REG_FIFO, varFlag) #
